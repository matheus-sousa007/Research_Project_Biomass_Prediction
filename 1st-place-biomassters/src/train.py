...
def train(args, model_logs):
    ...
    start_building_model = time()

    model = build_model(args)
    elapsed_time_building_model = time() - start_building_model

    start_loading_model_to_cuda = time()

    model = model.cuda()

    elapsed_time_loading_model_to_cuda = time() - start_loading_model_to_cuda

    model_logs.loc[len(model_logs)] = {
        "epoch": 0,
        "loss": float("inf"),
        "time_elapsed": elapsed_time_building_model,
        "obs": "Building the model",
    }
    ...
    model_logs.loc[len(model_logs)] = {
        "epoch": 0,
        "loss": float("inf"),
        "time_elapsed": elapsed_time_loading_model_to_cuda,
        "obs": "Loading model to GPU (cuda)",
    }
    ...
    start_building_optimizer = time()

    optimizer = build_optimizer(parameters, args)

    elapsed_time_building_optimizer = time() - start_building_optimizer

    model_logs.loc[len(model_logs)] = {
        "epoch": 0,
        "loss": float("inf"),
        "time_elapsed": elapsed_time_building_optimizer,
        "obs": "Building optimizer",
    }
...
def main():
    ...
    train(args, model_logs)

    model_logs.to_csv(Path(args.checkpoint_dir) / "model_logs.csv")
